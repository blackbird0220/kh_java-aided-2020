...
public class ArrayList {
	public static void main(String[] args) {
		...
		for(int i=0;i<ipInfoList.size();i++) {
			if(ipInfoList.get(i)==ip) {	//ipInfoList.get(i)는 특정번째의 IPInfo클래스의 객체이고, ip는 String 변수 이므로
				//if(ipInfoList.get(i).getIp().equals(ip)){로 고쳐야 한다.
				System.out.println(ipInfoList.get());
				break;
			}/* else {
